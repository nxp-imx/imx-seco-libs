...
typedef struct {
    ...
    uint8_t *iv;                                //!< pointer to the initialization vector\n(for i.MX8DXL, user supplied part of nonce in case of AES CCM)\n(for other devices, nonce in case of AES CCM)
    uint16_t iv_size;                           //!< length in bytes of the initialization vector\n it must be 0 for algorithms not using the initialization vector.\n for i.MX8DXL, it must be 0 or 4 for AES in CCM encryption mode depending on IV option\n for other devices or i.MX8DXL decryption, it must be 12 for AES in CCM mode
    ...
    uint8_t *output;                            //!< pointer to the output area\n ciphertext for encryption\n(for i.MX8DXL, in case of CCM is the output of the generation-encryption process + full IV)\n(for other devices, in case of CCM is the output of the generation-encryption process)\n plaintext for decryption
 ...
 *
 * For CCM encryption operations on i.MX8DXL, either HSM_CIPHER_ONE_GO_FLAGS_GENERATE_FULL_IV or HSM_CIPHER_ONE_GO_FLAGS_GENERATE_COUNTER_IV must be set when calling this function:
 * - When HSM_CIPHER_ONE_GO_FLAGS_GENERATE_FULL_IV is set, the full IV is internally generated, iv and iv_size must be set to 0
 * - When HSM_CIPHER_ONE_GO_FLAGS_GENERATE_COUNTER_IV is set, the user supplies a 4 byte fixed part of the IV.  The other IV bytes are internally generated
 *
...
#define HSM_CIPHER_ONE_GO_ALGO_AES_CCM              ((hsm_op_cipher_one_go_algo_t)(0x04u))       //!< Perform AES CCM with following constraints: AES CCM where Adata = 0, Tlen = 16 bytes, for i.MX8DXL nonce size depends on IV option, for other devices, nonce = 12 bytes
...
#define HSM_CIPHER_ONE_GO_FLAGS_GENERATE_FULL_IV    ((hsm_op_auth_enc_flags_t)(1u<< 1))     //!< Full IV is internally generated (only relevant for CCM encryption on i.MX8DXL)
#define HSM_CIPHER_ONE_GO_FLAGS_GENERATE_COUNTER_IV ((hsm_op_auth_enc_flags_t)(1u << 2))    //!< User supplies 4 bytes of the IV (fixed part), the other bytes are internally generated (only relevant for CCM encryption on i.MX8DXL)
...
#define HSM_AUTH_ENC_ALGO_AES_GCM              ((hsm_op_auth_enc_algo_t)(0x00u))       //!< Perform AES GCM with following constraints: AES GCM where AAD supported, Tag len = 16 bytes, IV len depends on IV option for encryption, IV len = 12 bytes for decryption
