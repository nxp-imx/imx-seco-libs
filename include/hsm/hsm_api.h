...
typedef struct {
    ...
    uint8_t *output;                            //!< pointer to the output area\n ciphertext for encryption (in case of CCM is the output of the generation-encryption process + full IV)\n plaintext for decryption
 ...
 *
 * For CCM encryption operations, either HSM_CIPHER_ONE_GO_FLAGS_GENERATE_FULL_IV or HSM_CIPHER_ONE_GO_FLAGS_GENERATE_COUNTER_IV must be set when calling this function:
...
#define HSM_CIPHER_ONE_GO_ALGO_AES_CCM              ((hsm_op_cipher_one_go_algo_t)(0x04u))       //!< Perform AES CCM with following constraints: AES CCM where Adata = 0, Tlen = 16 bytes, nonce size depends on IV option
...
#define HSM_CIPHER_ONE_GO_FLAGS_GENERATE_FULL_IV    ((hsm_op_auth_enc_flags_t)(1u << 1))    //!< Full IV is internally generated (only relevant for CCM encryption)
#define HSM_CIPHER_ONE_GO_FLAGS_GENERATE_COUNTER_IV ((hsm_op_auth_enc_flags_t)(1u << 2))    //!< User supplies 4 bytes of the IV (fixed part), the other bytes are internally generated (only relevant for CCM encryption)
...
#define HSM_AUTH_ENC_ALGO_AES_GCM              ((hsm_op_auth_enc_algo_t)(0x00u))       //!< Perform AES GCM with following constraints: AES GCM where AAD supported, Tag len = 16 bytes, IV len depends on IV option
