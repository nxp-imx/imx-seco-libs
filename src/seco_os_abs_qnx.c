...
struct seco_os_abs_hdl {
    int32_t     fd;
    uint32_t    type;
    pthread_t   rx_buf_tid;
};

/* MU devices root path */
static char SECO_MU_DEV_PATH[] = "/dev";
...
static char SECO_NVM_SHE_STORAGE_FILE[] = "/tmp/seco_she_nvm";
static char SECO_NVM_HSM_STORAGE_FILE[] = "/tmp/seco_hsm/seco_nvm_master";
static char SECO_NVM_HSM_STORAGE_CHUNK_PATH[] = "/tmp/seco_hsm/";
...
static char V2X_NVM_SHE_STORAGE_FILE[] = "/tmp/v2x_she_nvm";
static char V2X_NVM_HSM_STORAGE_FILE[] = "/tmp/v2x_hsm/v2x_nvm_master";
static char V2X_NVM_HSM_STORAGE_CHUNK_PATH[] = "/tmp/v2x_hsm/";

void *buffer_transfer_thread(void *data)
{
    struct seco_os_abs_hdl *phdl =  (struct seco_os_abs_hdl *)data;
    struct seco_mu_ioctl_setup_iobuf    iobuf;
    io_devctl_t     io_hdr;
    iov_t           iov[2];
    int32_t         error;
...
struct seco_os_abs_hdl *seco_os_abs_open_mu_channel(uint32_t type, struct seco_mu_params *mu_params)
{
    ...
    if ((phdl != NULL) && (device_path != NULL) && (mu_params != NULL)) {
        ...
        if (phdl != NULL) {
            pthread_create( &phdl->rx_buf_tid, NULL, buffer_transfer_thread, phdl);
            phdl->type = type;
...
uint32_t seco_os_abs_has_v2x_hw(void)
{
    ...
    dir_ptr = opendir(SECO_MU_DEV_PATH);
    if( dir_ptr != NULL ) {
        for(;;) {
            dirent_ptr = readdir(dir_ptr);
            if (dirent_ptr == NULL) { break; }
            if (strstr(V2X_MU_SV0_PATH, dirent_ptr->d_name) != NULL) {
                ret = 1U;
                break;
            }
        }
...
{
    /*
     * Nothing to do on QNX. The SCU RPC is automatically called at boot time.
